    ...
    <section className="container py-16">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {locale === "en" ? "Academic email required and unexpired ID." : "يلزم بريد جامعي وهوية غير منتهية."}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={validate} className="grid gap-3 md:grid-cols-3">
            ...
            <Input required type="date" value={expiry} onChange={(e) => setExpiry(e.target.value)} placeholder={t("expiry")} />
            <Button type="submit" disabled={loading}>{t("validate")}</Button>
          </form>
          {result && (
            <div className="mt-4 text-sm">
              {result.valid ? (
                <div className="text-emerald-600">
                  {locale === "en" ? `Valid. Discount ~${result.discountPercent}% applied at checkout.` : `صالح. سيت�� تطبيق خصم ~${result.discountPercent}% عند الدفع.`}
                </div>
              ) : (
